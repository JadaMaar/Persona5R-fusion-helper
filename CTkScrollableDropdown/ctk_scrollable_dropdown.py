...
class CTkScrollableDropdown(customtkinter.CTkToplevel):
    ...
    def __init__(self, attach, x=None, y=None, button_color=None, height: int = 200, width: int = None,
                 ...
                 scrollbar=True, scrollbar_button_hover_color=None, frame_border_width=2, values: list[Persona] = [],
    ...
    def _new_update(self, sv):
        self.live_update(sv.get())
    ...
    def bind_autocomplete(self, ):
        ...
        if self.attach.winfo_name().startswith("!ctkentry"):
            ...
            # self.var_update.trace_add('write', self._update)
            self.var_update.trace("w", lambda name, index, mode, sv=self.var_update: self._new_update(self.var_update))
    ...
    def _init_buttons(self, **button_kwargs):
        ...
        for row in self.values:
            self.widgets[self.i] = customtkinter.CTkButton(self.frame,
                                                           ...
                                                           anchor=self.justify,
                                                           command=lambda k=row.name: self._attach_key_press(k),
                                                           **button_kwargs)
    ...
    def live_update(self, string=None):
        ...
        if string:
            ...
            for key in self.widgets.keys():
                # changed to lowercase to be more lenient on the inputs
            ...
            for widget in self.widgets.values():
                widget.pack_forget()
            for widget in self.widgets.values():
                widget.pack(fill="x", pady=2, padx=(self.padding, 0))
            self.place_dropdown()
